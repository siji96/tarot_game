...
export default function TarotGame() {
  ...
  useEffect(() => {
    console.log("✅ tarotDeckData loaded:", tarotDeckData);
    setTarotDeck(tarotDeckData);
  }, []);

  const drawCards = async () => {
    if (tarotDeck.length === 0) {
      alert("🔄 Deck is still loading or empty.");
      return;
    }
    ...
    try {
      const response = await fetch("https://tarotgame-production.up.railway.app/api/save-draw", {
        method: "POST",
        headers: {
